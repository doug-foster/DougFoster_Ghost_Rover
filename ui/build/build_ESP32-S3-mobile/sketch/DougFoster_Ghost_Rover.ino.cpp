 ...
 *  browser <-- {"ver":"3.0.12 - Feb 19 2026 @ 12:46:28","prfUnt":"meter","prfRtcIn":"radio","prfNmeOut":"on","prfHotSsi":"doug","prfHotPas":"snark217$",
 *               "prfGnsNavRat":2,"prfGnsMsrInt":50,"}.
 *  -- Echo. --
 *  browser --> {"echo":"hello/etc"}.
 *  browser <-- {"echo":"hello/etc"}.
 *  -- Remove. --
 *  browser --> {"main/files/config":"ready"}. // ToDo: Remove. Replaced by {"page":"xxx"}.
 *  browser <-- {"main/files/config":"ready"}. jsonDocToClient["server"]
 * 
 * 
 ...
 *  <-- hello -->
 *  -- Remove. --
 *  browser --> {"cfg":"get"}. // ToDo: Remove
 *  browser <-- {"prfUnt":"meter","prfRtcIn":"radio","prfNmeOut":"on","prfHotSsi":"doug","prfHotPas":"snark217$", "prfGnsNavRat":2,"prfGnsMsrInt":50,
 *               "cfg":"Preferences retrieved."}.
 *  -- NMEA sentences. --
 ...
 *  -- Remove. --
 *  browser --> {"cfg":"get"}.
 *  browser <-- {"prfUnt":"meter","prfRtcIn":"radio","prfNmeOut":"on","prfHotSsi":"doug","prfHotPas":"snark217$",
 *               "prfGnsNavRat":2,"prfGnsMsrInt":50,"cfg":"Preferences retrieved."}
 *  -- Set preferences. --
 *  browser --> {"cfg":"set","prfUnt":"meter","prfRtcIn":"radio","prfNmeOut":"on","prfGnsMsrInt":"50","prfGnsNavRat":"2","prfHotSsi":"doug","prfHotPas":"snark217$"}
 *      checkPrefs(prefAction)
 *          jsonDocToClient["prfUnt"]       = prfUnt;
 *          jsonDocToClient["prfRtcIn"]     = prfRtcIn;
 *          jsonDocToClient["prfNmeOut"]    = prfNmeOut;
 *          jsonDocToClient["prfHotSsi"]    = prfHotSsi;
 *          jsonDocToClient["prfHotPas"]    = prfHotPas;
 *          jsonDocToClient["prfGnsNavRat"] = prfGnsNavRat;
 *          jsonDocToClient["prfGnsMsrInt"] = prfGnsMsrInt;
 *  <-- {"cfg":"Preference values updated."}
 *  -- Reset preferences. --
 *  browser --> {"cfg":"reset"}.
 *  browser <-- browser {"cfg":"message"}.  jsonDocToClient["cfg"] = "Preferences reset to defaults.";
 * 
 * 
 ...
 *  <-- hello -->
 *   browser --> {"opr":"?"}.
 *   browser <-- oper data (@see operDataToJsonDoc)
 *       operDataToJsonDoc();  // ToDo: Remove both sides. Now pushed not pulled in checkZED( *    * browser --> {"page":"operate"} // ToDo: Remove
 *  browser <-- {"page":"operate"}
 *  -- Get values. --
 *  browser --> server {"cfg":"get"}
 *  browser <-- {"prfUnt":"meter","prfRtcIn":"radio","prfNmeOut":"on","prfHotSsi":"doug","prfHotPas":"snark217$",
 *               "prfGnsNavRat":2,"prfGnsMsrInt":50,"cfg":"Preferences retrieved.","siv":19,"hgt-elip":"-26.05",
 *               "hgt-orth":"10.01","lat":"35.44422813","lon":"-76.92306668","hac":"2.013","vac":"3.122","rtcm":"d",
 *               "bt":"u","bat":"102.72","batc":"-1.0","up-tm":"3h 4m 54s","rtcm-cnt-all":1,"rtcm-rate":4,
 *               "nmea-cnt-gga":92515,"nmea-cnt-rmc":92515,"nmea-cnt-gsa":154185,"nmea-cnt-gsv":262667,"nmea-cnt-gst":30837,
 *               "nmea-cnt-txt":4,"nmea-cnt-othr":0,"nmea-cnt-all":632723,"nmea-rate":474112,"mode":"r","l-ip":"192.168.23.1",
 *               "h-ip":"172.20.10.2","ver":"3.0.12 - Feb 19 2026 @ 12:46:28"}
 *      checkZed() calls operDataToJsonDoc() which uses:
 *          jsonDocToClient["fix"]
 *          jsonDocToClient["siv"]
 *          jsonDocToClient["hgt-elip"]
 *          jsonDocToClient["hgt-orth"]
 *          jsonDocToClient["lat"]
 *          jsonDocToClient["lon"]
 *          jsonDocToClient["hac"]
 *          jsonDocToClient["vac"]
 *          jsonDocToClient["rtcm"]
 *          jsonDocToClient["bt"]  
 *          jsonDocToClient["bat"]
 *          jsonDocToClient["batc"]
 *          jsonDocToClient["up-tm"]
 *          jsonDocToClient["rtcm-cnt-all"]   = 1;
 *          jsonDocToClient["rtcm-rate"]      = 4;
 *          jsonDocToClient["nmea-cnt-gga"]   = nmeaCountGGA;
 *          jsonDocToClient["nmea-cnt-rmc"]   = nmeaCountRMC;
 *          jsonDocToClient["nmea-cnt-gsa"]   = nmeaCountGSA;
 *          jsonDocToClient["nmea-cnt-gsv"]   = nmeaCountGSV;
 *          jsonDocToClient["nmea-cnt-gst"]   = nmeaCountGST;
 *          jsonDocToClient["nmea-cnt-txt"]   = nmeaCountTXT;
 *          jsonDocToClient["nmea-cnt-othr"]  = nmeaCountOther;
 *          jsonDocToClient["nmea-cnt-all"]   = nmeaCountAll;
 *          jsonDocToClient["nmea-rate"]      = nmeaRate;
 *          jsonDocToClient["mode"]           = operMode;
 *          jsonDocToClient["l-ip"]           = localIp;
 *          jsonDocToClient["h-ip"]           = hotspotIp;
 *  -- Lock/unlock buttons. --
...
JsonDocument jsonDocToClient;                       // JSON document (jsonDocToClient) sent to client is a global var.
enum wsKeyID {                                      // Readable index for WwebSocket keys. @see Global vars. - WebSockets in global.js.
    ...
    WS_PREF_GNSS_NAV_RATE,                          //  5 - jsonDocToClient["prfGnsNavRat"]  --> jsonDocToClient[wsKey(wWS_PREF_GNSS_NAV_RATE)].
    ...
    WS_WIFI_LOCAL_NETWORK_IP,                       // 33 - jsonDocToClient["l-ip"]          --> jsonDocToClient[wsKey(WWS_WIFI_LOCAL_NETWORK_IP)].
 ...
 * @see    
 */
#line 544 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void statusLedOn();
#line 738 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
...
#line 910 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void showBuild();
...
void startSerial();
#line 986 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void initPins();
#line 1012 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void startI2C();
...
void startWiFi();
#line 1173 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void startSD();
#line 1235 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
...
#line 1325 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void startAndConfigGNSS();
#line 1405 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void startTasks();
#line 1421 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void preLoop();
#line 1458 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void taskLoopStatusLed(void * pvParameters);
#line 1498 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void onWiFiEvent(arduino_event_id_t event);
#line 1530 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void onHttpFileUpload(AsyncWebServerRequest *request, String filename, size_t index, uint8_t *data, size_t len, bool final);
#line 1576 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
...
#line 2016 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void checkZED();
#line 2081 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void relaySerial1toSerial2();
#line 2152 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
...
void checkSerialUSB();
#line 2247 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void debug();
#line 2421 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void checkGnssLockButton();
#line 2450 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void setup();
#line 2477 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void loop();
#line 544 "/Users/dougfoster/Library/CloudStorage/Dropbox/Data/doug/Topics/_dev-arduino/DougFoster_Ghost_Rover/DougFoster_Ghost_Rover.ino"
void statusLedOn() {
 ...
 */
void checkPrefs(prefAction action, const char* key = NULL, const char* value = NULL) {

    // --- Preference defaults. ---
    const char      NAMESPACE[]     = "config";     // The preference namespace. 
    const char      PRF_UNT[]       = "meter";      // Distance units: meter/feet (used only in browser).   Matching global var: char     prfUnt[6].
    const char      PRF_RTC_IN[]    = "radio";      // Control RTCM in: off/radio/ntrip.                    Matching global var: char     prfRtcIn[6].
    const char      PRF_NME_OUT[]   = "on";         // Control NMEA out: off/on.                            Matching global var: char     prfNmeOut[4].
    const char      PRF_HOT_SSI[]   = "ssid";       // WiFi hotspot client: network SSID.                   Matching global var: char     prfHotSsi[20].
    const char      PRF_HOT_PASS[]  = "pass";       // WiFi hotspot client: password.                       Matching global var: char     prfHotPas[30].
    const uint8_t   PRF_GNS_NAV_RAT = 2;            // ZED rate (times/interval): OUTPUT a new solution.    Matching global var: uint8_t  prfGnsNavRat.
    const uint16_t  PRF_GNS_MSR_INT = 100;          // ZED interval (ms): CREATE a new solution.            Matching global var: uint16_t prfGnsMsrInt.
    const uint16_t  numprefs        = 8;            // Number of preferences being used.
    ...
    switch (action) {
        case PREF_INIT:                                                     // Only called by setup().
            ...
            hasKey = roverPrefs.isKey("prfUnt");                            // Check for preference.
            roverPrefs.end();                                               // Close NAMESPACE object.

            // -- Read or reset? --
            if(hasKey) {
                checkPrefs(PREF_READ);                                      // Test preference exists, so they all should. Read values from NVS & set global vars.
            } else {
                checkPrefs(PREF_RESET);                                     // If the test preference doesn't exist, none of them do.
            }

            // -- Wrap up. --
            Serial.printf("NVS namespace %s using %u entries with %u available.\n", NAMESPACE, numprefs, roverPrefs.freeEntries());
            break;
        ...
        case PREF_READ:
            ...
            roverPrefs.getString("prfUnt",       prfUnt,    sizeof(prfUnt));
        ...
        case PREF_SAVE:
            ...
            // -- Process KV pairs. --
            if (strcmp(key, "prfUnt") == 0) {                               // Preference prfUnt.
                roverPrefs.begin("config", RW_MODE);                        // Open namespace object for read/write.
                strcpy(prfUnt, value);                                      // Update global var.
                roverPrefs.putString("prfUnt", prfUnt);                     // Save preference.
            } else if (strcmp(key, "prfRtcIn") == 0) {                      // Preference prfRtcIn.
                strcpy(prfRtcIn, value);
                roverPrefs.putString("prfRtcIn", prfRtcIn);
            } else if (strcmp(key, "prfNmeOut") == 0) {                     // Preference prfNmeOut.
                strcpy(prfNmeOut, value);
                roverPrefs.putString("prfNmeOut", prfNmeOut);
            } else if (strcmp(key, "prfHotSsi") == 0) {                     // Preference prfHotSsi.
                strcpy(prfHotSsi, value);
                roverPrefs.putString("prfHotSsi", prfHotSsi);
            } else if (strcmp(key, "prfHotPas") == 0) {                     // Preference prfHotPas.
                strcpy(prfHotPas, value);
                roverPrefs.putString("prfHotPas", prfHotPas);
            } else if (strcmp(key, "prfGnsNavRat") == 0) {                  // Preference prfGnsNavRat.
            ...
            } else if (strcmp(key, "prfGnsMsrInt") == 0) {                  // Preference prfGnsMsrInt.
                prfGnsMsrInt = (uint16_t) atoi(value);
            ...
            if (jsonPairNum == 1) {
                roverPrefs.end();                                           // Close NAMESPACE object.
                jsonDocToClient["cfg"] = "Preference values updated.";
        ...
        case PREF_RESET:
            ...
            strcpy(prfUnt,    PRF_UNT);                                     // Set global vars to defaults.
            strcpy(prfRtcIn,  PRF_RTC_IN);
            strcpy(prfNmeOut, PRF_NME_OUT);
            strcpy(prfHotSsi, PRF_HOT_SSI);
            strcpy(prfHotPas, PRF_HOT_PASS);
            prfGnsNavRat = PRF_GNS_NAV_RAT;
            prfGnsMsrInt = PRF_GNS_MSR_INT;
        ...
        case PREF_PRINT:
            ...
            Serial.printf( "prfUnt       \"%s\", \"%s\", \"%s\"\n", PRF_UNT,         prfUnt,       roverPrefs.getString("prfUnt"));
            Serial.printf( "prfRtcIn     \"%s\", \"%s\", \"%s\"\n", PRF_RTC_IN,      prfRtcIn,     roverPrefs.getString("prfRtcIn"));
            Serial.printf( "prfNmeOut    \"%s\", \"%s\", \"%s\"\n", PRF_NME_OUT,     prfNmeOut,    roverPrefs.getString("prfNmeOut"));
            Serial.printf( "prfHotSsi    \"%s\", \"%s\", \"%s\"\n", PRF_HOT_SSI,     prfHotSsi,    roverPrefs.getString("prfHotSsi"));
            Serial.printf( "prfHotPas    \"%s\", \"%s\", \"%s\"\n", PRF_HOT_PASS,    prfHotPas,    roverPrefs.getString("prfHotPas"));
            Serial.printf( "prfGnsNavRat %u, %u, %u\n",             PRF_GNS_NAV_RAT, prfGnsNavRat, roverPrefs.getUChar("prfGnsNavRat"));
            Serial.printf( "prfGnsMsrInt %u, %u, %u\n",             PRF_GNS_MSR_INT, prfGnsMsrInt, roverPrefs.getUShort("prfGnsMsrInt"));
            roverPrefs.end();                                               // Close NAMESPACE object.
            break;

        case PREF_TO_JSON:
            // @see browser --> {"cfg":"get"} in onWebSocketMessage().
            // browser <-- browser {"prfUnt":"meter","prfRtcIn":"radio","prfNmeOut":"on","prfGnsMsrInt":"100","prfGnsNavRat":"2","prfGnsMsrInt":"280","prfHotSsi":"ssid","prfHotPas":"pass","cfg":"message"}.
...
void operDataToJsonDoc() {
    ...
    if (roverGNSS.getSIV() > MIN_SATELLITE_THRESHHOLD) {        // Enough satellites?
        ...
        jsonDocToClient["hgt-elip"] = numberbuffer;
        ...
        jsonDocToClient["hgt-orth"] = numberbuffer;
        ...
        // --- RTCM & BT status. ---
        jsonDocToClient["rtcm"] = (RTCMin)  ? "u" : "d";                    // Up, down. @see relaySerial1toSerial2().
        jsonDocToClient["bt"]   = (NMEAout) ? "u" : "d";                    // Up, down. @see DevUBLOXGNSS::processNMEA().
        ...
        jsonDocToClient["up-tm"] = uptime;
        // WebSocket status items are calculated in operate.js.
        jsonDocToClient["rtcm-cnt-all"]   = 1;
        jsonDocToClient["rtcm-rate"]      = 4;
        jsonDocToClient["nmea-cnt-gga"]   = nmeaCountGGA;
        jsonDocToClient["nmea-cnt-rmc"]   = nmeaCountRMC;
        ...
        jsonDocToClient["lon"]  = 0;
        jsonDocToClient["hgt"]  = 0;
        jsonDocToClient["hac"]  = 0;
        jsonDocToClient["vac"]  = 0;
        jsonDocToClient["rtcm"] = 0;
        jsonDocToClient["bt"]   = 0;
    }
}
 ...
 *
 * Example: jsonDocToClient[wsKey(WS_VERSION)] = someValue;.
 *          jsonDocToClient["0"].
 *
 * @param  int id Key ID #.
 ...
 * @see   checkPrefs(PREF_INIT)  - Check preferences.
 ...
 * @see    Global vars: GNSS, checkPrefs(), startSerial(), beginI2C().
 ...
 * @see    Global vars: GNSS, checkPrefs(), onWebSocketEvent(), startWebSocketServer().
 * @link   https://randomnerdtutorials.com/esp32-websocket-server-arduino/.
...
void onWebSocketMessage(void *arg, uint8_t *data, size_t len) {
    ...
    // --- Local vars. ---
    const  size_t WS_FULL_SEND_COUNT = 20;
    static bool   configSet          = false;                       // {"cfg":"set"} flag: multiple kv preference pairs in message.
    ...
    if (info->final && info->index == 0 && info->len == len && info->opcode == WS_TEXT) {
        ...
        // -- Create JSON KV pairs. --
        memset(JSONbuffer, '\0', sizeof(JSONbuffer));
        serializeJson(jsonDocFromClient, JSONbuffer, sizeof(JSONbuffer));
        jsonPairNum = jsonDocFromClient.as<JsonObject>().size();   // Used to close preferences.
        ...
        // -- Process each JSON KV pair. --
        for (JsonPair kv : jsonDocFromClient.as<JsonObject>()) {
            jsonDocToClient.clear();
            char key[20];
            strcpy(key, kv.key().c_str());                          // Improve code readability.
            char value[50];
            strcpy(value, kv.value().as<const char*>());            // Improve code readability.
            ...
            if ((strcmp(key, "page") == 0)) {
                ...
                strcpy(whichPage, value);
                jsonDocToClient["page"]                                = value;         // Echo back.
            ...
            //  browser <-- oper data (@see operDataToJsonDoc)
            // --------------------------------------------
            else if ((strcmp(key, "opr") == 0) && (strcmp(value, "?") == 0 )) {
                // operDataToJsonDoc();  // Remove. Now pushed not pulled in checkZED().
            }
            ...
            // --------------------------------------------
                ...
                jsonDocToClient["server"] = value;   // Echo ready.
            }
            ...
            // --------------------------------------------
            else if ((strcmp(value, "lock") == 0) || (strcmp(value, "unlock") == 0)) {
                
                // - Laser pointer button. -
                //   @link https://www.build-electronic-circuits.com/arduino-laser-module-ky-008/.
                //   @link https://docs.sparkfun.com/SparkFun_Thing_Plus_ESP32-S3/arduino_example/#rgb-led.
                if (strcmp(key, "laser") == 0) {
                    if (strcmp(value, "lock") == 0) {                          
                        digitalWrite(LSR_TRIGGER, HIGH);        // Turn on.
                    }
                    if (strcmp(value, "unlock") == 0) { 
                        digitalWrite(LSR_TRIGGER, LOW);         // Turn off.
                    }
                }
                ...
                if ((strcmp(key, "laser") == 0) ||
                    ...
                    memset(JSONbuffer, '\0', sizeof(JSONbuffer));
                    strcpy(JSONbuffer, value);
                    strcat(JSONbuffer, "ed");
                    jsonDocToClient[key] = JSONbuffer;
                }
            }
            ...
            // --------------------------------------------
            else if (strcmp(key, "listFiles") == 0) {
                char output[2048];
            ...
            // browser <-- {"deleteFile":"fileDeleted/fileNOTdeleted"}.
            // --------------------------------------------
            else if (strcmp(key, "deleteFile") == 0) {
                if (SD.remove(value)) {  // Delete file.
                    jsonDocToClient["fileDeleted"] = value;
                } else {
                    jsonDocToClient["fileNOTdeleted"] = value;
                }
            }
            ...
            // browser <-- No reply.
            // --------------------------------------------
            else if ((strcmp(key, "mcu") == 0) && (strcmp(value, "restart") == 0 )) {
            ...
            // --------------------------------------------
            // browser --> {"cfg":"get"}.
            // browser <-- browser {preference KV pairs}
            // --------------------------------------------
            else if ((strcmp(key, "cfg") == 0) && (strcmp(value, "get") == 0)) {
                checkPrefs(PREF_TO_JSON);   // Send new preference values back to browser.
                jsonDocToClient["cfg"] = "Preferences retrieved.";
            }

            // --------------------------------------------
            // bbrowser --> {"cfg":"reset"}.
            // browser <-- browser {"cfg":"message"}.
            // --------------------------------------------
            else if ((strcmp(key, "cfg") == 0) && (strcmp(value, "reset") == 0)) {
                checkPrefs(PREF_RESET);     // Reset preferences to defaults.
                checkPrefs(PREF_TO_JSON);   // Send new preference values back to browser.
                jsonDocToClient["cfg"] = "Preferences reset to defaults.";
            }

            // --------------------------------------------
            // browser --> {preference KV pairs}
            // browser <-- browser {"cfg":"message"}.
            // --------------------------------------------
            else if ((strcmp(key, "cfg") == 0) && (strcmp(value, "set") == 0)) {
                configSet = true;
            }
            if (configSet) {
                checkPrefs(PREF_SAVE, key, value);
            }

            // - Send one JSON reply after all KV pairs have been processed. -
            if (jsonPairNum == 1) {
                configSet = false;
                memset(JSONbuffer, '\0', sizeof(JSONbuffer));
            ...
            jsonPairNum--;      // Track # of JSON kv pairs.
        }
    }
}
...
void setup() {
    ...
    checkPrefs(PREF_INIT);      // Check preferences.
